...
- 登录电脑后自动启动看板服务。
- 周一至周五按交易节奏自动刷新：9:32、10:30、11:25、13:30、14:15、14:35、14:45、14:50、14:55、15:10、16:10。
- 14:15 进入午后预选，14:35 进入尾盘候选，14:50 生成第一版推荐，14:55 用最新行情二次校验并生成最终推荐，同时执行模拟盘买入窗口。
- 自动卖出和 14:55 模拟买入都必须先通过数据新鲜度闸门：行情快照必须是当天数据，且距离当前时间不超过 10 分钟；A 池买入还需要实时二次确认价格、成交量、成交额、换手率、量比。
